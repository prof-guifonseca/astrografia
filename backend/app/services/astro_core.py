...
# 📍 Caminho absoluto para efemérides (compatível com Render e execução local)
...
def grau_para_signo(degree):
    ...
    if not isinstance(degree, (float, int)):
        raise TypeError(f"Grau inválido recebido: {degree}")
    index = int(degree // 30) % 12
    return SIGNOS[index], round(degree % 30, 2)

def calcular_mapa(birth_date, birth_time, lat, lon):
    """
    Calcula posições planetárias e ascendente com base na data/hora/local.
    ...
    if not all([birth_date, birth_time, isinstance(lat, (float, int)), isinstance(lon, (float, int))]):
        raise ValueError("Parâmetros incompletos ou inválidos.")

    try:
        year, month, day = map(int, birth_date.split('-'))
        hour, minute = map(int, birth_time.split(':'))
    except Exception as e:
        raise ValueError("Data ou hora com formato incorreto. Use 'YYYY-MM-DD' e 'HH:MM'.") from e
    ...
    for nome, pid in PLANETAS.items():
        try:
            lon_planeta = swe.calc_ut(jd_ut, pid)[0][0]  # longitude
    ...
    try:
        ...
        print(f"⚠️ Erro ao calcular o ascendente: {e}")
        ascendente = {
            'sign': 'Desconhecido',
            'degree': 0.0
        }
